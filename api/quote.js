// The ten stocks at the top of $SPY (and $SPY itself) from Yahoo Finance's public chart endpoint: last price, previous close, today's 15-minute closes.
// If Yahoo refuses the server, the response says so and carries the last snapshot we read (2026-09-18 close) stamped with its date — never a made-up number.
const LIST = ['SPY','NVDA','AAPL','MSFT','AMZN','GOOGL','AVGO','GOOG','META','MU','TSLA'];
const NAMES = { SPY:'S&P 500 ETF', NVDA:'NVIDIA', AAPL:'Apple', MSFT:'Microsoft', AMZN:'Amazon', GOOGL:'Alphabet A', AVGO:'Broadcom', GOOG:'Alphabet C', META:'Meta Platforms', MU:'Micron', TSLA:'Tesla' };
// index weights read 2026-09-18
export const WEIGHTS = { NVDA:8.08, AAPL:7.03, MSFT:5.69, AMZN:3.84, GOOGL:3.01, AVGO:2.65, GOOG:2.40, META:1.90, MU:1.63, TSLA:1.56 };
// snapshot 2026-09-18 after the close: [price, previous close, last 12 daily closes]
const SNAP = {
  SPY:[761.69,null,[]],  // only the close we read; no invented history
...
export default async function handler(req, res) {
  ...
  await Promise.all(tks.map(async tk => {
    try { const q = await yahoo(tk); quotes[tk] = { ...q, name: NAMES[tk], weight: WEIGHTS[tk] || null, live: true }; live++; latest = Math.max(latest, q.t || 0); }
    catch (e) { const s = SNAP[tk]; quotes[tk] = { price: s[0], prev: s[1], closes: s[2], name: NAMES[tk], weight: WEIGHTS[tk] || null, live: false, asof: '2026-09-18' }; }
  }));
  res.setHeader('cache-control', 'public, s-maxage=60, stale-while-revalidate=300');
  return res.status(200).json({ ok: true, list: LIST, weights: WEIGHTS, asof_weights: '2026-09-18', quotes, live, of: tks.length, via: live ? 'Yahoo Finance chart API' : 'snapshot 2026-09-18 (Yahoo refused the server read)', t: latest || null });
}
